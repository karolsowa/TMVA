...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
   ...
   TMVA::Factory *factory = new TMVA::Factory("TMVAClassification", outputFile,"!V:!Color:!DrawProgressBar:Transformations=I;D;P;G,D:AnalysisType=Classification");
   ...
   factory->BookMethod(dataloader, TMVA::Types::kBDT, "BDT", "!H:!V:VarTransform=N,G,G_Signal,G_Background:NTrees=" + NTrees_str + ":MinNodeSize=" + MinNodeSize_str + ":MaxDepth=" + MaxDepth_str + ":BoostType=AdaBoost:AdaBoostBeta=" + AdaBoostBeta_str + ":UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=100");
   ...
   // AdaBoostBeta_str = std::to_string(AdaBoostBeta);
   // MaxDepth_str = std::to_string(MaxDepth);
   // MinNodeSize_str = std::to_string(MinNodeSize);
   // NTrees_str = std::to_string(NTrees);


   //factory->BookMethod(dataloader, TMVA::Types::kBDT, "BDT_opt", "!H:!V:VarTransform=N,G,G_Signal,G_Background:NTrees=" + NTrees_str + ":MinNodeSize=" + MinNodeSize_str + ":MaxDepth=" + MaxDepth_str + ":BoostType=AdaBoost:AdaBoostBeta=" + AdaBoostBeta_str + ":UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=100");
   ...
   //"BDT" - user-defined name (can be different)
   //H
   //V
   ...
   //AUC_tab[0] = factory->GetROCIntegral(dataloader, "BDT");
   //AUC_tab[1] = factory->GetROCIntegral(dataloader, "BDT_opt");
   // AUC_tab[2] = factory->GetROCIntegral(dataloader, "PDERS");
   // AUC_tab[3] = factory->GetROCIntegral(dataloader, "PDEFoam");

   // std::string AUC_str[4];
   // for(int ind = 0; ind < 4; ind++)
   //    AUC_str[ind] = std::to_string(AUC_tab[ind]);
   ...
   //TString weightfile = dir + prefix + "_BDT.weights.xml";
   TString BDT_weights = dir + prefix + "_BDT.weights.xml";
   //TString BDT_opt_weights = dir + prefix + "_BDT_opt.weights.xml";
   TString PDERS_weights = dir + prefix + "_PDERS.weights.xml";
   TString PDEFoam_weights = dir + prefix + "_PDEFoam.weights.xml";   



   reader->BookMVA("BDT method", BDT_weights);
   //reader->BookMVA("BDT_opt method", BDT_opt_weights);
   ...
   for (Long64_t ievt=0; ievt<nEvent; ievt++) {
      ...
      //BDT_response = reader->EvaluateMVA("BDT_opt method");
   ...
   // std::cout<<"BDT: "<<AUC_str[0]<<std::endl;
   // //std::cout<<"BDT opt: "<<AUC_str[1]<<std::endl;
   // std::cout<<"PDERS: "<<AUC_str[2]<<std::endl;
   // std::cout<<"PDEFoam: "<<AUC_str[3]<<std::endl;
